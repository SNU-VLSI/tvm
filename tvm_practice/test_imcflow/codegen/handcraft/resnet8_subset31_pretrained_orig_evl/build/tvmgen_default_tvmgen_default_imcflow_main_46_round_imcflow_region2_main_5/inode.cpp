...
void tvmgen_default_tvmgen_default_imcflow_main_46_round_imcflow_region2_main_5() {
  ...
  if (hid == 0 && wid == 0) { // inode_0_0
    ...
    for (int i1 = 0; i1 < 61; i1++) { // generate
    ...
    // generate: send - TensorEdge(((58, -41), config), ((58, 53), config)), inode_0_0 -> imce_0_1
    var4 = 8192;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 4, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((58, -41), config), ((58, 53), config)), inode_0_0 -> imce_0_1
    // generate: send - TensorEdge(((58, -42), fused_scale), ((58, 54), fused_scale)), inode_0_0 -> imce_0_1
    var4 = 8224;
    for (int i1 = 0; i1 < 2; i1++) { // generate
      __builtin_INODE_SEND(var4 + i1*32, 0, 1, 1);

    } // endgenerate
    // endgenerate: send - TensorEdge(((58, -42), fused_scale), ((58, 54), fused_scale)), inode_0_0 -> imce_0_1
    // generate: send - TensorEdge(((58, -43), fused_bias), ((58, 54), fused_bias)), inode_0_0 -> imce_0_1
    var4 = 8288;
    for (int i1 = 0; i1 < 2; i1++) { // generate
      __builtin_INODE_SEND(var4 + i1*32, 0, 2, 1);

    } // endgenerate
    // endgenerate: send - TensorEdge(((58, -43), fused_bias), ((58, 54), fused_bias)), inode_0_0 -> imce_0_1
    // generate: send - TensorEdge((-44, min), (57, min)), inode_0_0 -> imce_0_2
    var4 = 8352;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 5, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-44, min), (57, min)), inode_0_0 -> imce_0_2
    // generate: send - TensorEdge((-45, max), (57, max)), inode_0_0 -> imce_0_2
    var4 = 8384;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 6, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-45, max), (57, max)), inode_0_0 -> imce_0_2
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(5, 255);
    __builtin_INODE_STANDBY(10, 255);
    ...
    __builtin_INODE_HALT();
    // endgenerate: halt for slave inodes
    // generate: send - TensorEdge((-31, odata), (56, lhs)), inode_0_0 -> imce_1_2
    var5 = (int*)(32768);
    var4 = 0;
    var6 = var5[0];
    __asm__ volatile("nop");
    for (int i1 = 0; i1 < var6; i1++) { // generate
      ...
      __builtin_INODE_SET_FLAG(0);
      __builtin_INODE_SEND(var4 + i1*32, 0, 7, 2);

    } // endgenerate
    // endgenerate: send - TensorEdge((-31, odata), (56, lhs)), inode_0_0 -> imce_1_2
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(5, 255);
    __builtin_INODE_STANDBY(10, 255);
    ...
    __builtin_INODE_SET_FLAG(0);
    // endgenerate: wait all imce compute enable
    // generate: send - TensorEdge((-46, min), (59, min)), inode_1_0 -> imce_1_1
    var4 = 0;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 1, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-46, min), (59, min)), inode_1_0 -> imce_1_1
    // generate: send - TensorEdge((-47, max), (59, max)), inode_1_0 -> imce_1_1
    var4 = 32;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 2, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-47, max), (59, max)), inode_1_0 -> imce_1_1
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(0, 255);
    __builtin_INODE_STANDBY(10, 255);
    ...
    __builtin_INODE_HALT();
    // endgenerate: halt for slave inodes
    // generate: send - TensorEdge((-32, odata), (56, rhs)), inode_1_0 -> imce_1_2
    var7 = (int*)(32768);
    var4 = 0;
    var8 = var7[0];
    __asm__ volatile("nop");
    ...
    // endgenerate: send - TensorEdge((-32, odata), (56, rhs)), inode_1_0 -> imce_1_2
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(0, 255);
    __builtin_INODE_STANDBY(10, 255);
    ...
    var2 = 17888;
    __builtin_INODE_SET_ADDR_CNT(0);
    for (int i1 = 0; i1 < 201; i1++) { // generate
      __builtin_INODE_WR_IMEM(var2 + i1*32, 0, 9);
    } // endgenerate
    ...
    __builtin_INODE_SET_FLAG(0);
    // endgenerate: wait all imce compute enable
    // generate: send - TensorEdge(((62, -36), config), ((62, 49), config)), inode_3_0 -> imce_3_1
    var4 = 16384;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 5, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((62, -36), config), ((62, 49), config)), inode_3_0 -> imce_3_1
    // generate: send - TensorEdge(((62, -37), fused_scale), ((62, 51), fused_scale)), inode_3_0 -> imce_3_1
    var4 = 16416;
    for (int i1 = 0; i1 < 2; i1++) { // generate
      __builtin_INODE_SEND(var4 + i1*32, 0, 2, 1);

    } // endgenerate
    // endgenerate: send - TensorEdge(((62, -37), fused_scale), ((62, 51), fused_scale)), inode_3_0 -> imce_3_1
    // generate: send - TensorEdge(((62, -38), fused_bias), ((62, 51), fused_bias)), inode_3_0 -> imce_3_1
    var4 = 16480;
    for (int i1 = 0; i1 < 2; i1++) { // generate
      __builtin_INODE_SEND(var4 + i1*32, 0, 3, 1);

    } // endgenerate
    // endgenerate: send - TensorEdge(((62, -38), fused_bias), ((62, 51), fused_bias)), inode_3_0 -> imce_3_1
    // generate: send - TensorEdge((-49, config), (61, config)), inode_3_0 -> imce_3_2
    var4 = 16544;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 7, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-49, config), (61, config)), inode_3_0 -> imce_3_2
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(0, 255);
    __builtin_INODE_STANDBY(5, 255);
