...
void tvmgen_default_tvmgen_default_imcflow_main_0_round_imcflow_region1_main_0() {
  ...
  int* var5; // sm10_d17data_send_data_base_address
  int var6; // sm10_d17data_tile_loop_count
  int* var7; // s20_12_d21func_out0_recv_data_base_address
  int var8; // s20_12_d21func_out0_tile_loop_count
  ...
  if (hid == 0 && wid == 0) { // inode_0_0
    ...
    __builtin_INODE_HALT();
    // endgenerate: halt for slave inodes
    // generate: send - TensorEdge((-10, odata), (17, data)), inode_0_0 -> imce_3_4
    var5 = (int*)(2048);
    var4 = 0;
    var6 = var5[0];
    __asm__ volatile("nop");
    ...
    // endgenerate: send - TensorEdge((-10, odata), (17, data)), inode_0_0 -> imce_3_4
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(5, 255);
    __builtin_INODE_STANDBY(10, 255);
    ...
    var2 = 23936;
    __builtin_INODE_SET_ADDR_CNT(0);
    for (int i1 = 0; i1 < 173; i1++) { // generate
    ...
    __builtin_INODE_SET_FLAG(0);
    // endgenerate: wait all imce compute enable
    // generate: send - TensorEdge(((20, -13), config), ((20, 11), config)), inode_3_0 -> imce_3_1
    var4 = 16384;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 12, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((20, -13), config), ((20, 11), config)), inode_3_0 -> imce_3_1
    // generate: send - TensorEdge(((20, -14), fused_scale), ((20, 12), fused_scale)), inode_3_0 -> imce_3_1
    var4 = 16416;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 6, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((20, -14), fused_scale), ((20, 12), fused_scale)), inode_3_0 -> imce_3_1
    // generate: send - TensorEdge(((20, -15), fused_bias), ((20, 12), fused_bias)), inode_3_0 -> imce_3_1
    var4 = 16448;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 10, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((20, -15), fused_bias), ((20, 12), fused_bias)), inode_3_0 -> imce_3_1
    // generate: send - TensorEdge((-23, min), (19, min)), inode_3_0 -> imce_3_2
    var4 = 16640;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 7, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-23, min), (19, min)), inode_3_0 -> imce_3_2
    // generate: send - TensorEdge((-24, max), (19, max)), inode_3_0 -> imce_3_2
    var4 = 16672;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 13, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-24, max), (19, max)), inode_3_0 -> imce_3_2
    // generate: send - TensorEdge(((18, -18), config), ((18, 14), config)), inode_3_0 -> imce_3_3
    var4 = 16480;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 16, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((18, -18), config), ((18, 14), config)), inode_3_0 -> imce_3_3
    // generate: send - TensorEdge(((18, -19), fused_scale), ((18, 15), fused_scale)), inode_3_0 -> imce_3_3
    var4 = 16512;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 8, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((18, -19), fused_scale), ((18, 15), fused_scale)), inode_3_0 -> imce_3_3
    // generate: send - TensorEdge(((18, -20), fused_bias), ((18, 15), fused_bias)), inode_3_0 -> imce_3_3
    var4 = 16544;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 14, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((18, -20), fused_bias), ((18, 15), fused_bias)), inode_3_0 -> imce_3_3
    // generate: send - TensorEdge((-21, min), (17, min)), inode_3_0 -> imce_3_4
    var4 = 16576;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 9, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-21, min), (17, min)), inode_3_0 -> imce_3_4
    // generate: send - TensorEdge((-22, max), (17, max)), inode_3_0 -> imce_3_4
    var4 = 16608;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 17, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-22, max), (17, max)), inode_3_0 -> imce_3_4
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(0, 255);
    __builtin_INODE_STANDBY(5, 255);
    ...
    __builtin_INODE_HALT();
    // endgenerate: halt for master inode after done and intrt
    // generate: recv: TensorID(21, func_out0)
    var7 = (int*)(8192);
    var9 = 0;
    var8 = var7[0];
    __asm__ volatile("nop");
    ...
    // endgenerate: recv: TensorID(21, func_out0)
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(0, 255);
    __builtin_INODE_STANDBY(5, 255);
