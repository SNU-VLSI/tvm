...
void tvmgen_default_tvmgen_default_imcflow_main_46_round_imcflow_region1_main_0() {
  ...
  int* var7; // s46_44_d47func_out1_split1_recv_data_base_address
  int var8; // s46_44_d47func_out1_split1_tile_loop_count
  int var9; // recv_data_base_address
  int* var10; // s42_d47func_out0_split0_recv_data_base_address
  int var11; // s42_d47func_out0_split0_tile_loop_count
  if (hid == 0 && wid == 0) { // inode_0_0
    ...
    // generate: send - TensorEdge(((46, -28), scale), ((46, 44), rhs)), inode_0_0 -> imce_0_1
    var4 = 64;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 4, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((46, -28), scale), ((46, 44), rhs)), inode_0_0 -> imce_0_1
    // generate: send - TensorEdge((-19, min), (38, min)), inode_0_0 -> imce_0_2
    var4 = 0;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 2, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-19, min), (38, min)), inode_0_0 -> imce_0_2
    // generate: send - TensorEdge((-20, max), (38, max)), inode_0_0 -> imce_0_2
    var4 = 32;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 3, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-20, max), (38, max)), inode_0_0 -> imce_0_2
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(5, 255);
    __builtin_INODE_STANDBY(10, 255);
    ...
    for (int i1 = 0; i1 < 3; i1++) { // generate
      // generate: send - TensorEdge((-13, odata), ((46, 43), data)), inode_0_0 -> imce_0_1
      var5 = (int*)(53248);
      var4 = 0;
      var6 = var5[0];
      __asm__ volatile("nop");
      ...
      // endgenerate: send - TensorEdge((-13, odata), ((46, 43), data)), inode_0_0 -> imce_0_1
      // generate: sync all inodes
      __builtin_INODE_SET_FLAG(255);
      __builtin_INODE_STANDBY(5, 255);
      __builtin_INODE_STANDBY(10, 255);
    ...
    __builtin_INODE_SET_FLAG(0);
    // endgenerate: wait all imce compute enable
    // generate: send - TensorEdge(((40, -15), fused_scale), ((40, 35), fused_scale)), inode_1_0 -> imce_1_1
    var4 = 8192;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 4, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((40, -15), fused_scale), ((40, 35), fused_scale)), inode_1_0 -> imce_1_1
    // generate: send - TensorEdge(((40, -16), fused_bias), ((40, 35), fused_bias)), inode_1_0 -> imce_1_1
    var4 = 8224;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 5, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((40, -16), fused_bias), ((40, 35), fused_bias)), inode_1_0 -> imce_1_1
    // generate: send - TensorEdge(((40, -17), min), ((40, 36), min)), inode_1_0 -> imce_1_1
    var4 = 8256;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 2, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((40, -17), min), ((40, 36), min)), inode_1_0 -> imce_1_1
    // generate: send - TensorEdge(((40, -18), max), ((40, 36), max)), inode_1_0 -> imce_1_1
    var4 = 8288;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 3, 1);

    // endgenerate
    // endgenerate: send - TensorEdge(((40, -18), max), ((40, 36), max)), inode_1_0 -> imce_1_1
    // generate: send - TensorEdge((-22, config), (39, config)), inode_1_0 -> imce_1_2
    var4 = 8320;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 7, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-22, config), (39, config)), inode_1_0 -> imce_1_2
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(0, 255);
    __builtin_INODE_STANDBY(10, 255);
    ...
    __builtin_INODE_SET_FLAG(0);
    // endgenerate: wait all imce compute enable
    // generate: send - TensorEdge((-24, config), (41, config)), inode_2_0 -> imce_2_1
    var4 = 8192;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 3, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-24, config), (41, config)), inode_2_0 -> imce_2_1
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(0, 255);
    __builtin_INODE_STANDBY(5, 255);
    ...
    // endgenerate: halt for slave inodes
    for (int i1 = 0; i1 < 3; i1++) { // generate
      // generate: recv: TensorID(47, func_out1)
      var7 = (int*)(53248);
      var9 = 0;
      var8 = var7[0];
      __asm__ volatile("nop");
      ...
      // endgenerate: recv: TensorID(47, func_out1)
      // generate: sync all inodes
      __builtin_INODE_SET_FLAG(255);
      __builtin_INODE_STANDBY(0, 255);
      __builtin_INODE_STANDBY(5, 255);
    ...
    __builtin_INODE_SET_FLAG(0);
    // endgenerate: wait all imce compute enable
    // generate: send - TensorEdge((-25, fused_scale), (42, fused_scale)), inode_3_0 -> imce_3_1
    var4 = 0;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 2, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-25, fused_scale), (42, fused_scale)), inode_3_0 -> imce_3_1
    // generate: send - TensorEdge((-26, fused_bias), (42, fused_bias)), inode_3_0 -> imce_3_1
    var4 = 32;
    // generate. loop count == 1
    __builtin_INODE_SEND(var4 + 0*32, 0, 3, 1);

    // endgenerate
    // endgenerate: send - TensorEdge((-26, fused_bias), (42, fused_bias)), inode_3_0 -> imce_3_1
    // generate: sync all inodes
    __builtin_INODE_SET_FLAG(255);
    __builtin_INODE_STANDBY(0, 255);
    __builtin_INODE_STANDBY(5, 255);
    ...
    // endgenerate: halt for master inode after done and intrt
    for (int i1 = 0; i1 < 3; i1++) { // generate
      // generate: recv: TensorID(47, func_out0)
      var10 = (int*)(45056);
      var9 = 0;
      var11 = var10[0];
      __asm__ volatile("nop");
      ...
      // endgenerate: recv: TensorID(47, func_out0)
      // generate: sync all inodes
      __builtin_INODE_SET_FLAG(255);
      __builtin_INODE_STANDBY(0, 255);
      __builtin_INODE_STANDBY(5, 255);
