...
CKPT=n0_signed_sample_fixed DEBUG_EXE=1 CONSOLE_LOG_LEVEL=INFO ./run_dataset_eval.sh 100
CKPT=n32_signed_sample DEBUG_EXE=1 CONSOLE_LOG_LEVEL=INFO ./run_dataset_eval.sh 100
# --------------------------------------------------------------------------------------------------------------------
...
# CKPT=mapware_t2_a0p5_w5_ndis32 \
CKPT=tmp01_refine_ndis32 \
python main.py --model resnet8_subset31_pretrained_orig --driver-v2 --ref-models transformed \
      --num-disable-columns 32 --column-disable-config /root/project/CIM/noise/noise_df/B2_out/N32/disabled.json --random-seed 42 \
      --noise-csv /root/project/CIM/noise/noise_df/B2_out/N32/B2_noise_matrix_per_ch_concat.csv \
      --noise-layout-json /root/project/CIM/noise/noise_df/B2_out/N32/concat_per_core.json \
...
python main.py --model resnet8_subset31_pretrained_orig --driver-v2 --ref-models transformed \
      --fixed-imce-core 0,1 \
      --num-disable-columns 32 --column-disable-config /root/project/CIM/noise/noise_df/B2_out_refine_fixed_full_v1_partial/N32/disabled.json --random-seed 42 \
      --stop-at compile

CKPT=n0_signed_sample_fixed \
python main.py --model resnet8_subset31_pretrained_orig --driver-v2 --ref-models transformed \
      --fixed-imce-core 0,1 \
      --num-disable-columns 0 --random-seed 42 \
      --stop-at compile

python main.py --model resnet8_subset31_pretrained_orig --driver-v2 --ref-models transformed \
      --num-disable-columns 32 --column-disable-config /root/project/CIM/noise/noise_df/B2_out/N32/disabled.json --random-seed 42 \
      --noise-csv /root/project/CIM/noise/noise_df/B2_out_refine/N32/B2_noise_matrix_per_ch_concat.csv \
      --noise-layout-json /root/project/CIM/noise/noise_df/B2_out_refine/N32/concat_per_core.json \
      --stop-at compile
...
python3 tvm_practice/test_imcflow/codegen/scripts/measure_weight3_0_synthetic_noise.py \
    ...
    --connection root@147.46.117.99:1326 \
    --board B2 \
    --scan-val 0x0a \
    --dda 1.13 --ddc 1.17 --ddl 0.006 --ddf 1.24 \
    --repeats 100

python3 scripts/verify_acc_mask_skip.py \
    --connection root@147.46.117.99:1326 \
    --board B2 \
    --scan-val 0x0a \
    --dda 1.13 --ddc 1.17 --ddl 0.006 --ddf 1.24 \
    --repeats 50 \
    --ps-remote 147.46.117.49:1329 \
    --vmode VMode.HALF

# --noise-csv /root/project/CIM/noise/noise_df/B2_out/N32/B2_noise_matrix_per_ch_concat__alpha0.5_w5_T2.0.csv \
python3 scripts/measure_weight3_0_synthetic_noise.py \
    --candidates debugging/noise_lowlevel/weight3_0/weight3_0_candidate_tuples.json \
    --connection root@147.46.117.99:1326 \
    --board B2 \
    --scan-val scan_gen/B2_opt_scan_reg_files/set.0325 \
    --dda 1.25 --ddc 1.30 --ddl 0.023 --ddf 0.670 \
    --ps-remote 147.46.117.49:1329 \
    --vmode VMode.HALF \
    --pairs-per-candidate 200 --repeats 50 \
    --noise-csv /root/project/CIM/noise/noise_df/B2_out_refine/N32/B2_noise_matrix_per_ch_concat__alpha0.5_w5_T2.0_exclude_ref0-16.csv \
    --recompute-summary
...
python3 scripts/diagnose_noise_per_qconv.py \
    --noise-dir /root/project/CIM/noise/noise_df/B2_out_refine_fixed_full_v1_partial/N32 \
    --noise-csv B2_noise_matrix_per_ch_concat__alpha0.5_w5_T2.0_exclude_ref0-78.csv \
    --ckpt-alias tmp02_refine_ndis32_0_78 \
    --ckpt-board B2 \
    --ckpt-vmode half \
    --n-samples 100 \
    --skip-diagnose
