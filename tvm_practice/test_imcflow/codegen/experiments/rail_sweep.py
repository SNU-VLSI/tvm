#!/usr/bin/env python3
"""Generic rail undervolt sweep (swept rails move TOGETHER at the same value) for the 16-imce 127x127 power kernel.
...
def main():
    ...
    ap.add_argument("--fix", action="append", default=[], metavar="RAIL=V")
    ap.add_argument("--sweep", default="VDD", help="comma list, same value applied")
    ...
    tag = "_".join(args.sweep.split(","))
    out_csv = args.out or os.path.join(CODEGEN, "experiments", f"sweep_{tag}_{date}.csv")
    ...
    meta = {
        ...
        "fixed": ";".join(args.fix) or "(V1)",
        "sweep_rails": args.sweep,
        "sweep": f"{args.sweep} {args.start}->{args.stop} step {args.step}",
    ...
    cols = ["idx", "time", "swept_v", "vdd_meas", "dda_meas", "ddc_meas",
    ...
    sweep_rails = [r.strip().upper() for r in args.sweep.split(",")]
    for spec in args.fix:
        rail, _, val = spec.partition("=")
        mgr.set_voltage(rail.strip().upper(), float(val))
        log(f"fixed {rail}={val}")
    ...
    try:
        for idx, v in enumerate(points):
            for r_ in sweep_rails:
                mgr.set_voltage(r_, v)
            ...
            for attempt in (1, 2):
                ...
                log(f"{args.sweep}={v}: attempt {attempt} failed -> {status}")
            ...
            if pulse and pulse["vdd"]["run_mA"] is not None:
                ...
                log(f"{args.sweep}={v}: FAILED twice -> stop sweep (record kept)")
                break
    finally:
        mgr.apply_preset("V1")
        ...
        w.writerow(["sanity", time.strftime("%H:%M:%S"), "V1", "", "", "",
