...
int main(int argc, char** argv) {
  ...
  // Optional: when sweeping a single binary over many dataset samples, the
  // host driver passes the active sample index here so input/output dirs get
  // a per-sample suffix (test_inputs/sample_<N>, test_outputs/<runner>/sample_<N>).
  // Empty string keeps the legacy single-sample layout.
  ...
  // Ensure output_dir (and any new per-sample sub-dir) exists before any
  // save_tensor_to_dir / fopen call.
  ...
  for (uint32_t nid = 0; nid < exec->op_execs_count; nid++) {
    ...
    if (exec->op_execs[nid].fexec) {
      fprintf(stderr, "  [%u] Executing: %s (op: %s)\n", nid, node_name, op_type);
      exec->op_execs[nid].Call(&(exec->op_execs[nid]));

      // Check for IMCFlow kernel timeout
      if (g_imcflow_kernel_failed) {
        fprintf(stderr, "  ⚠️  [%u] IMCFlow kernel timeout on node '%s' — continuing\n", nid, node_name);
      ...
      for (uint32_t out_idx = 0; out_idx < exec->nodes[nid].param.num_outputs; out_idx++) {
        ...
        // Build filename: <nid>_<node_name>[_<out_idx>]
        char fname[256];
        if (exec->nodes[nid].param.num_outputs == 1) {
          snprintf(fname, sizeof(fname), "%03u_%s", nid, node_name);
        } else {
