...
if [[ "$SKIP_STEP6" == true ]]; then
    ...
    REMOTE_CMD="cd $REMOTE_BASE_PATH && ${POWER_RUNTIME_ENV}${DAE_ENV}${TIMING_ENV}IMCFLOW_DEBUG_DUMP_DIR=$CHIP_DEBUG_DUMP_DIR IMCFLOW_HEARTBEAT_PATH=$CHIP_HEARTBEAT_PATH taskset -c $CHIP_EVAL_CPU $BINARY_DIR/build/$DATASET_EXEC_NAME \
$GRAPH_PATH \
$PARAMS_PATH \
$IMAGES_PATH \
$LABELS_PATH \
