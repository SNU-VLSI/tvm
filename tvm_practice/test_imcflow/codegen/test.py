...
MODEL_REGISTRY = {
    ...
    "one_dwconv_v1": (lambda: models_for_test.getOneDWConvModel(iH=4,iW=4, IC=16, random=False), "random"),
    "one_dwconv_v2": (lambda: models_for_test.getOneDWConvModel(iH=4,iW=4, IC=32, random=True), "random"),
    "one_dwconv_v3": (lambda: models_for_test.getOneDWConvModel(iH=4,iW=4, IC=64, random=True), "random"),
    "one_dwconv_v4": (lambda: models_for_test.getOneDWConvModel(iH=32,iW=32, IC=64, random=True), "random"),
...
def run_test(test_name, eval_dir, mod, param_dict, options: PipelineOptions, input_data_dict=None, npz_file=""):
...
# Test Pipeline
# ============================================================================
def run_test_pipeline(test_name: str, options: PipelineOptions, npz_file=""):
  """
  Test pipeline that:
  ...
  with tee_output_to_log(log_file_path):
    ...
    run_test(test_name, dir_name, mod, param_dict, options=options, input_data_dict=input_dict, npz_file=npz_file)
...
def test_imcflow_model_with_pattern(test_name, input_pattern, is_default, setup_cache):
  ...
  run_test_pipeline(test_name, options=options, npz_file=npz_file)
