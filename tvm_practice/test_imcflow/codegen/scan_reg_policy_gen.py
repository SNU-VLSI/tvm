...
class ScanRegPolicyGenerator:
    ...
    def allocate(self, func_name: str = "scan_reg") -> None:
        ...
        # Register policy table and NoC paths to DevConfig
        ImcflowDeviceConfig().PolicyTableDict[func_name] = self.PolicyTable_2D
        ImcflowDeviceConfig().NoCPaths[func_name] = self.NoCPaths
        print(f"Policy table registered to ImcflowDeviceConfig().PolicyTableDict['{func_name}']")
        print(f"NoC paths registered to ImcflowDeviceConfig().NoCPaths['{func_name}']")
