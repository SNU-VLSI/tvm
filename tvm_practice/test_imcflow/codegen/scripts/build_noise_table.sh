...
#
# Usage:
#   ./scripts/build_noise_table.sh [run_dir] [samples]
#
# Examples:
#   # Default: latest run, samples 0-100
#   CKPT=uqat_tmp02_refine_ndis32_0_78 ./scripts/build_noise_table.sh
#
#   # Specific run and sample range
#   CKPT=uqat_tmp02_refine_ndis32_0_78 ./scripts/build_noise_table.sh run_00 0-199
#
# Environment:
#   CKPT          (required) checkpoint alias
#   BOARD         chip board name for default paths/checkpoint resolve (default: B2)
...
#   NOISE_TABLE_LAYER_SPECIFIC set to 1 to also build per-layer tables (default: 0)
...
NOISE_TABLE_LAYER_SPECIFIC="${NOISE_TABLE_LAYER_SPECIFIC:-0}"
NOISE_DIR="${NOISE_DIR:-/root/project/CIM/noise/noise_df/${BOARD}_out/N32}"
...
CKPT_PATH="${CKPT_PATH:-}"
if [[ -z "$CKPT_PATH" ]]; then
    CKPT_PATH=$(BOARD_NAME="$BOARD" CKPT_ALIAS="$CKPT" python3 -c "
import os, sys; sys.path.insert(0, '/root/project/CIM')
from checkpoints import resolve
print(resolve(os.environ['BOARD_NAME'], 'half', os.environ['CKPT_ALIAS'])[0])
")
fi

# Output paths
CIM_NOISE_DIR="${OUTPUT_DIR:-/root/project/CIM/noise/noise_df/${BOARD}_chip_inference/N32/${CKPT}}"
...
if [[ "$WRITE_CSV" == "1" && -z "$CSV_OUTPUT" ]]; then
    CSV_OUTPUT="$CIM_NOISE_DIR/${BOARD}_noise_matrix_per_ch_concat_signed_weight_ref.csv"
fi
...
echo "  Layer table: $NOISE_TABLE_LAYER_SPECIFIC"
...
case "$NOISE_TABLE_LAYER_SPECIFIC" in
