...
def test_layout_transform_NCHW_to_NCHW16c():
    print("\n" + "="*60)
    print("Test: layout_transform_NCHW_to_NCHW16c")
    print("="*60)
    ...
    for dtype in dtypes:
        ...
        expected_output = np.pad(x_data, ((0,0),(0,pad_size),(0,0),(0,0)), mode='constant').reshape(1, cg_size, 16, shape_nchw[-2], shape_nchw[-1]).transpose(0,1,3,4,2)
        print(f"------------- ours -------------")
        print(output)
        print(f"------------- expect -------------")
