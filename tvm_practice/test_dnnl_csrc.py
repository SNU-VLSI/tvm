    ...
    for mode in ["graph"]:
        ...
        for use_dnnl, alter_layout, use_bf16 in configs:
            ...
            with tvm.transform.PassContext(opt_level=3):
                ...
                json, lib, param = relay.build(processed_mod, target=target, params=params)
                lib = update_lib(lib)
                rt_mod = tvm.contrib.graph_executor.create(json, lib, dev)
            ...
            if run_module:
                if isinstance(input, dict):
                    ...
                    rt_mod.run()
                    result_dict[result_key] = rt_mod.get_output(0)
